...
# 1. Fix the Navigation Button
content = re.sub(
    r'<button onclick="App\.switchTab\(\'microlearning\'\)" id="tab-btn-microlearning".*?</button>',
...
# 2. Fix the View Section
# We need to find the entire <section id="view-microlearning"> ... </section>
# It ends right before <!-- VISTA 3: MATRIZ DE DEFENSA -->
match = re.search(r'<section id="view-microlearning" class="hidden space-y-6">.*?(?=<!-- VISTA 3: MATRIZ DE DEFENSA -->)', content, re.DOTALL)
...
if match:
  ...
  '''
    content = content[:match.start()] + new_view + content[match.end():]
    print("HTML view replaced!")
else:
    print("Could not find view-microlearning in HTML.")
